...
def pingpong(n):
    ...
    True
    """
    # 用recur func的argument来记录状态
    return helper(n, 1, 1, 1)
# the ith value is cur
def helper(n, i, cur, direction):
    # direction = 1 or -1
    if i == n:
        return cur
    if switch(i):
        direction *= -1
    return helper(n, i + 1, cur + direction, direction)
...
# combination which eliminates duplicates 
def count_coins(change):
    ...
    True
    """
    return coins_helper(1, change)
def coins_helper(coin, remain):
    if remain == 0:
        return 1
    if remain < 0:
        return 0
    if not coin:
        return 0
    with_coin = coins_helper(coin, remain - coin)
    nextcoin = next_larger_coin(coin)
    without_coin = coins_helper(nextcoin, remain)
